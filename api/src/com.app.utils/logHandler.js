...
const textLog = __dirname + appcfg.application.logs.type.debug.path;
...
var textStream = fs.createWriteStream(textLog, { encoding: 'utf-8' });
...
Logger.textFile = function(obj) {
    var date = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, ''),
        message = 'File Name:' + obj.fileName + '\nBatch Id:' + obj.batchId + '\nDate:' + date + '\n' + obj.errorRecords + '\n';

    console.log('\n');
    console.log("=============Debug Logging ===========") + '\n';
    console.log("===========Start============= " + date) + '\n';
    console.log("Record: " + obj.errorRecords) + '\n';
    console.log("===========End============= " + date) + '\n';

    textStream.write(message);
};

Logger.console = function(msg, mthd) {
    var date = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
    switch (mthd) {
        case undefined:
        case "":
